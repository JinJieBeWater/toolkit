...
下表覆盖当前 `settings.json.packages` 中全部 30 个 package。资源类型来自 package manifest；“禁用”表示 package 已安装，但对应 extension 不加载。
...
| 所有权路由 | `git:github.com/JinJieBeWater/toolkit`      | Pi Loom 0.3.0 | Extension                  | 启用           | 管理任务所有权、持久 helper 与 managed worktree             |
