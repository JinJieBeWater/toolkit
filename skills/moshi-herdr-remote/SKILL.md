...
description: "Moshi + Herdr 移动端远程接入：通过 EasyTier 私网连接 macOS，持续操作 Pi 或其他受支持 CLI Agent。"
---

# Moshi + Herdr 移动端远程接入

目标链路：移动设备 → EasyTier 私网 → Moshi SSH/Mosh → Herdr 持久 pane → Agent。
...
- 目标 Agent，默认 `pi`。
- 用户可操作支持 EasyTier 私网与 Moshi 的移动设备，并完成配对。本文的 EasyTier 客户端安装步骤仅提供 Android 已验证示例。
...
完成标准：`easytier-cli -p 127.0.0.1:15888 peer` 能看到 Mac 和移动设备，并已记录 `<MAC_EASYTIER_IP>`。
...
完成标准：移动设备能通过 `<MAC_EASYTIER_IP>:22` 建立终端，`moshi-hook host list` 有该主机，`moshi-hook status` 显示 paired、daemon 可达、目标 Agent hook 为 current。
...
1. 移动设备启动 EasyTier 并保持后台连接。Android 已验证示例：开启 VPN，电池策略设为“不限制”。
2. Moshi 连接 `<MAC_NAME> via EasyTier`。
3. 进入 Herdr session，向目标 Agent 发送一次无副作用 prompt。
4. 移动设备断网后恢复，再进入同一 pane。

完成标准：移动设备可输入、查看输出和处理审批；断线重连后原 pane 与 Agent 仍在。`moshi-hook context` 在该 pane 中返回 `kind=herdr`。
