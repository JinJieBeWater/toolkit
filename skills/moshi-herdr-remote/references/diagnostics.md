...
2. **Mesh**：执行 [EasyTier 组网](easytier.md)末尾验证，确认本机与手机节点都存在。Android 节点消失时先检查 VPN 和电池限制。
3. **SSH/Mosh**：从 EasyTier 网络测试 `<MAC_EASYTIER_IP>:22`。Android TUN 可能不回 ICMP，ping 不能判定失败；Mosh 报 `os error 111` 时先确认手机仍在 peer 列表。
